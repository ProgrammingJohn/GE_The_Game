...
from .speed_challenge_game import SpeedChallenge
...
class Player(BasePlayer):
    ...
    def _assign_to_minigame(self, minigame_name: str) -> None:
        if minigame_name not in self._get_minigame_names():
            raise ValueError(f'Please match an appropriate minigame name: Minigame names: {
                             self._get_minigame_names()}')
    ...
    def _start_minigame(self, minigame_name): self._get_minigame_start_map()[minigame_name]()
    ...
    def _get_current_minigame_name(
        self) -> str: return self.current_minigame_name

    def _get_current_maze(self): return self._get_mazes()[-1]

    def _get_current_speed_challenge(
        self): return self._get_speed_challenges()[-1]
    ...
    def _get_minigame_names(self) -> set: return {
        minigame_name for minigame_name, _ in self._get_minigame_start_map().items()}

    def _get_mazes(self) -> list[Maze]: return self.mazes

    def _get_speed_challenges(
        self) -> list[SpeedChallenge]: return self.speed_challenges
    def _get_minigames(self) -> list[Maze,
                                     SpeedChallenge]: return self.minigames

    def _get_minigame_start_map(self) -> dict: return self.minigame_start_map

    def _set_minigame_name(
        self, minigame_name: str = None) -> None: self.current_minigame_name = minigame_name
    ...
    def _add_speed_challenge(self) -> None:
        ...
        self._get_speed_challenges().append(
            new_speed_challenge), self._get_minigames(new_speed_challenge)

    def _add_minigame(self, minigame: (Maze, SpeedChallenge)
                      ) -> None: self._get_minigames().append(minigame)
